...
export const renderLoadElements = (data: DataDto, config: sunsynkPowerFlowCardConfig) => {
	...
	return html`
		...
		<svg id="Load" style="overflow: visible; border: 1px solid red;" x="${config.wide ? '30%' : '3%'}" y="2.5%">
			...
			${renderPath('es-load1', 'M 412 80 L 412 60', !showAux && additionalLoad === 1, data.load1Colour, 1)}
			${renderPath(
				'es-load2',
				'M 412 80 L 412 53',
			...
			<a
				href="#"
				@click=${(e) => (config.load.navigate ? Utils.handleNavigation(e, config.load.navigate) : null)}
			>
				<svg
				...
				>
					...
					<path
						display="${[1, 2].includes(additionalLoad) && !showAux ? '' : 'none'}"
				...
				>
					...
					<path
						display="${[4, 5, 6].includes(additionalLoad) && !showAux ? '' : 'none'}"
						fill="${dynamic_colour ? `url(#Lg-${data.timestamp_id})` : loadColour}"
						d="${data.essIcon}"
					/>
				</svg>
				...
				>
					...
					<path
						display="${[1, 2, 3, 4, 5, 6].includes(additionalLoad) ? 'none' : ''}"
						fill="${dynamic_colour ? `url(#Lg-${data.timestamp_id})` : loadColour}"
						d="${data.essIcon}"
					/>
				</svg>
