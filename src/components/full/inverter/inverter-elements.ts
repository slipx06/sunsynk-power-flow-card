...
export const renderInverterElements = (data: DataDto, inverterImg: string, config: sunsynkPowerFlowCardConfig) => {
	const {inverterColour, largeFont, enableAutarky, enableTimer, priorityLoad} = data;
	...
	return html`
		...
		<svg id="Inverter" style="overflow: visible" x="${config.wide ? '20%' : '3%'}" y="2.5%">
			<rect
				...
				pointer-events="all"
			/>
			<text x="167" y="306" class="st3 left-align" fill="${inverterColour}">${data.inverterStateMsg}</text>
			${renderText(
				'autarkye_value',
				212,
				283,
