...
export const renderGridElements = (data: DataDto, config: sunsynkPowerFlowCardConfig) => {
	...
	return html`
		...
		>
			...
			<svg id="ne1-flow">
				${renderPath('ne-line1', 'M 339 295 L 339 310', showNonessential, gridColour, data.nonessLineWidth)}
				${renderCircle(
					'ne-dot1',
			...
			<svg id="ne-flow">
				${renderPath('ne-line', 'M 339 265 L 339 188', showNonessential, gridColour, data.nonessLineWidth)}
				${renderCircle(
					'ne-dot',
			...
			<g display="${!showNonessential || [1, 2, 3].includes(nonessentialLoads) ? 'none' : ''}">
				${renderIcon(undefined, data.nonessentialIcon, 'noness-icon', 303.5, 303.5, 85, 85)}
			</g>
			<g display="${!showNonessential || [0, 1].includes(nonessentialLoads) ? 'none' : ''}">
				${renderIcon(undefined, data.iconNonessentialLoad1, 'nonessload1-icon', 306, 341)}
			</g>
			<g display="${!showNonessential || [0, 1].includes(nonessentialLoads) ? 'none' : ''}">
				${renderIcon(undefined, data.iconNonessentialLoad2, 'nonessload2-icon', 343, 341)}
			</g>
			<g display="${!showNonessential || [0, 2, 3].includes(nonessentialLoads) ? 'none' : ''}">
				${renderIcon(undefined, data.iconNonessentialLoad1, 'nonessload1-icon', 324.5, 341)}
			</g>
			${createTextWithPopup(
