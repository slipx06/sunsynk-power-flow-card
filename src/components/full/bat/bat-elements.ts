    ...
    return html`
        ...
        <svg id="Battery" 
            ...
            <svg id="battery_total_power" 
                style="display: ${config.wide && batteryCount === 2 ? 'inline' : 'none'};">
            ...
            <svg id="battery1_remaining" 
                style="overflow: visible; display: ${batteryCount === 1  ? 'inline' : 'none'};">
                ${renderText(
                    'duration_text',
                    132,
                    368,
            ...
            <svg id="battery2_remaining"
                ...
                <text x="144"
                    ...
                    fill="${battery2Colour}">${data.battery2StateMsg}
                </text>
            </svg>
            <svg id="battery1_data" 
                x="${batteryCount === 2 ? '16.25%' : '0%'}">
                <rect x="6" y="${batteryCount === 2 ? '320.75' : '300.75'}" width="${batteryCount === 2 ? 40 : 70}" height="${batteryCount === 2 ? 50 : 70}" 
                    rx="${batteryCount === 2 ? 4.5 : 10.5}" ry="${batteryCount === 2 ? 4.5 : 10.5}" fill="none"
                    stroke="${batteryColour}" pointer-events="all"/>
                ${createTextWithPopup(
                    'battery_voltage_183',
                ...
                ${createTextWithPopup(
                    ...
                    `${show_absolute
                        ...
                        : Utils.toNum(data.stateBatteryCurrent.state, 1)} ${UnitOfElectricalCurrent.AMPERE}`,
            ...
            <svg id="battery2_data" 
                ...
                ${createTextWithPopup(
                    ...
                    `${config.battery2.show_absolute
                        ...
                        : Utils.toNum(data.stateBattery2Current.state, 1)} ${UnitOfElectricalCurrent.AMPERE}`,
            ...
            <svg id="battery-flow">
                ...
                ${renderCircle(
                    'power-dot-charge',
                    Math.min(2 + data.batLineWidth + Math.max(data.minLineWidth - 2, 0), 8),
                    data.batteryPowerTotal > 0 || data.batteryPowerTotal === 0 ? 'transparent' : (config.battery.dynamic_colour ? data.flowBatColour : batteryColour),
                    data.durationCur['battery'],
                    "0;1",
                    "#bat-line",
                    config.battery.invert_flow
