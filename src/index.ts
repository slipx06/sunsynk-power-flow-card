...
export class SunsynkPowerFlowCard extends LitElement {
  ...
  render() {
    ...
    let load_showdaily = config?.load?.show_daily || false;
    let grid_colour = config?.grid?.colour || '#5490c2';
    let no_grid_colour = config?.grid?.no_grid_colour || '#a40013';
    let grid_show_noness = config?.grid?.show_nonessential || true;
    ...
    if (noness_dual_load !== false && noness_dual_load !== 1 && noness_dual_load !== 2) {
      noness_dual_load = false;
    }

    let grid_showdailybuy = config?.grid?.show_daily_buy || false;
    let grid_showdailysell = config?.grid?.show_daily_sell || false;
    let battery_colour = config?.battery?.colour || 'pink';
    let battery_showdaily = config?.battery?.show_daily || false;
    let solar_colour = config?.solar?.colour || 'orange';
    let solar_showdaily = config?.solar?.show_daily || false;

    let show_aux = config?.load?.show_aux;
    if (show_aux !== false && show_aux !== true) {
      show_aux = false;
    }

    let additional_load = config?.load?.additional_loads;
    if (additional_load !== false && additional_load !== 1 && additional_load !== 2) {
    ...
    let font = config?.large_font || false;
    let panel = config?.panel_mode || false;
    ...
    if (config.cardstyle === 'full') {
      return html`
        ...
        <div class="container card">
          ...
          <svg viewBox="-0.5 -0.5 457 383" preserveAspectRatio="xMidYMid meet" height="${panel === false ? `${height}` : '100%'}" width="100%" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <rect x="237" y="32" width="70" height="30" rx="4.5" ry="4.5" fill="none" stroke="${aux_colour}" pointer-events="all" class="${show_aux === false ? 'st12' : ''}"/>
            ...
            <rect id="es-load1" x="374" y="${show_aux === false ? '30' : '143'}" width="70" height="30" rx="4.5" ry="4.5" fill="none" stroke="${load_colour}" pointer-events="all" display="${additional_load === 1 ? '' : 'none'}"/>
            <rect id="es-load2" x="${show_aux === false ? '376' : '373'}" y="${show_aux === false ? '30' : '143'}" width="35" height="20" rx="4.5" ry="4.5" fill="none" stroke="${load_colour}" pointer-events="all" display="${additional_load === 2 ? '' : 'none'}"/>
            <rect id="es-load2" x="${show_aux === false ? '413' : '410'}" y="${show_aux === false ? '30' : '143'}" width="35" height="20" rx="4.5" ry="4.5" fill="none" stroke="${load_colour}" pointer-events="all" display="${additional_load === 2 ? '' : 'none'}"/>

            <text id="duration" x="29%" y="92%" class="${font === false ? 'st14' : 'st4'} left-align" fill="${config.battery.energy === 0 || float === 'True' || battery_power === 0 ? 'transparent' : `${battery_colour}`}" >${duration}</text>
            <text id="duration_text" x="29%" y="96%" class="st3 left-align" fill="${config.battery.energy === 0 || battery_power <= 0 || float === 'True' ? 'transparent' : `${battery_colour}`}" >${localize('common.runtime_to')} ${battery_capacity}% @${formattedResultTime}</text>
            <text id="duration_text_charging" x="29%" y="96%" class="st3 left-align" fill="${config.battery.energy === 0 || battery_power >= 0 || float === 'True' ? 'transparent' : `${battery_colour}`}" >${localize('common.to')} ${battery_capacity}% ${localize('common.charge')} @${formattedResultTime}</text>
            <text id="floating" x="29%" y="96%" class="st3 left-align" fill="${config.battery.energy === 0 || float === 'False' ? 'transparent' : `${battery_colour}`}" >${localize('common.battery_floating')}</text>
            <text x="-0.25%" y="20.5%" class="st3 st8 left-align" display="${config.show_solar === false ? 'none' : ''}" fill="${solar_colour}">${config?.solar?.pv1_name ? `${config.solar.pv1_name}` : 'PV1'}</text>
            <text x="21.75%" y="20.5%" class="st3 st8 left-align" display="${config.show_solar === false || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}">${config?.solar?.pv2_name ? `${config.solar.pv2_name}` : 'PV2'}</text>
            <text x="-0.25%" y="36.25%" class="st3 st8 left-align" display="${config.show_solar === false || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}">${config?.solar?.pv3_name ? `${config.solar.pv3_name}` : 'PV3'}</text>
            <text x="21.75%" y="36.25%" class="st3 st8 left-align" display="${config.show_solar === false || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}">${config?.solar?.pv4_name ? `${config.solar.pv4_name}` : 'PV4'}</text>
            ...
            <text x="90%" y="41%" class="st3 st8" display="${(additional_load === 1 || additional_load === 2) && show_aux === true ? 'none' : ''}" fill="${load_colour}">Essential</text>
            <text id="ess_load" x="90%" y="34%" class="st3 st8" display="${additional_load === false || show_aux === false ? 'none' : ''}" fill="${load_colour}">Essential</text>
            <text id="ess-load1" x="416" y="${show_aux === false ? '70' : '181'}" class="st3 left-align" display="${additional_load === 1 ? '' : 'none'}" fill="${load_colour}" >${config?.load?.load1_name ? `${config.load.load1_name}` : ''}</text>
            <text id="ess_load1" x="409" y="${show_aux === false ? '47' : '158'}" display="${additional_load === 1 ? '' : 'none'}" class="${font === false ? 'st14' : 'st4'} st8" fill="${load_colour}">${isNaN(parseFloat(stateObj42.state)) ? 0 : parseFloat(stateObj42.state).toFixed(0)} W</text>
            <text id="ess-load2" x="${show_aux === false ? '393' : '390'}" y="${show_aux === false ? '59' : '172'}" class="st3 st8" display="${additional_load === 2 ? '' : 'none'}" fill="${load_colour}" >${config?.load?.load1_name ? `${config.load.load1_name}` : ''}</text>
            <text id="ess_load2" x="${show_aux === false ? '394' : '391'}" y="${show_aux === false ? '42' : '154'}" display="${additional_load === 2 ? '' : 'none'}" class="st3" fill="${load_colour}">${isNaN(parseFloat(stateObj42.state)) ? 0 : parseFloat(stateObj42.state).toFixed(0)} W</text>
            <text id="ess-load2" x="${show_aux === false ? '429' : '426'}" y="${show_aux === false ? '59' : '172'}" class="st3 st8" display="${additional_load === 2 ? '' : 'none'}" fill="${load_colour}" >${config?.load?.load2_name ? `${config.load.load2_name}` : ''}</text>
            <text id="ess_load2" x="${show_aux === false ? '430' : '427'}" y="${show_aux === false ? '42' : '154'}" display="${additional_load === 2 ? '' : 'none'}" class="st3" fill="${load_colour}">${isNaN(parseFloat(stateObj45.state)) ? 0 : parseFloat(stateObj45.state).toFixed(0)} W</text>
            <text id="daily_load_aux" x="67%" y="24.25%" class="st3 left-align" fill="${load_showdaily === false || show_aux === false ? 'transparent' : `${load_colour}`}" >${localize('common.daily_load')}</text>
            <text id="daily_load" x="${additional_load === false ? '83%' : '67%'}" y="${additional_load === false ? '18.5%' : '24.25%'}" class="st3 left-align" fill="${load_showdaily === false || show_aux === true ? 'transparent' : `${load_colour}`}" >${localize('common.daily_load')}</text>
            <text id="daily_solar" x="43.5" y="7.5%" class="st3 left-align" fill="${solar_showdaily === false || config.show_solar === false || remaining_solar != 'false' ? 'transparent' : `${solar_colour}`}" >${localize('common.daily_solar')}</text>
            <text id="remaining_solar" x="43.5" y="7.5%" class="st3 left-align" fill="${solar_showdaily === false || config.show_solar === false || remaining_solar === 'false' ? 'transparent' : `${solar_colour}`}" >DAILY SOLAR / LEFT TODAY</text>
            <text id="daily_bat_charge" x="1%" y="65.5%" class="st3 left-align"  fill="${battery_showdaily === false ? 'transparent' : `${battery_colour}`}" >${localize('common.daily_charge')}</text>
            <text id="daily_bat_discharge" x="1%" y="74%" class="st3 left-align"  fill="${battery_showdaily === false ? 'transparent' : `${battery_colour}`}" >${localize('common.daily_discharge')}</text>
            <text id="daily_grid_buy" x="${grid_show_noness === false ? '68%' : '76%'}" y="${grid_show_noness === false ? '96%' : '66%'}" class="st3 left-align" fill="${grid_showdailybuy === false ? 'transparent' : `${grid_colour}`}" >${localize('common.daily_grid_buy')}</text>
            <text id="daily_grid_sell" x="${grid_show_noness === false ? '68%' : '76%'}" y="${grid_show_noness === false ? '88%' : '58%'}" class="st3 left-align" fill="${grid_showdailysell === false ? 'transparent' : `${grid_colour}`}" >${localize('common.daily_grid_sell')}</text>
            <text x="90%" y="21.5%" class="st3 st8" display="${show_aux === false ? 'none' : ''}" fill="${aux_status === 'on' || aux_status === '1' ? `${aux_colour}` : `${aux_off_colour}`}" >${config?.load?.aux_name ? `${config.load.aux_name}` : 'Auxiliary'}</text>
            <text id="noness" x="74.5%" y="98.5%" class="st3 st8" display="${grid_show_noness === false ? 'none' : ''}" fill="${grid_colour}">${config?.grid?.nonessential_name ? `${config.grid.nonessential_name}` : 'Non Essential'}</text>
            <text id="noness1" x="340"" y="338" class="st3 st8" display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 2 ? 'none' : ''}" fill="${grid_colour}">${config?.grid?.load1_name ? `${config.grid.load1_name}` : ''}</text>
            <text id="noness2" x="321"" y="338" class="st3 st8" display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}">${config?.grid?.load1_name ? `${config.grid.load1_name}` : ''}</text>
            <text id="noness2" x="358" y="338" class="st3 st8" display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}">${config?.grid?.load2_name ? `${config.grid.load2_name}` : ''}</text>
            <text id="noness1_value" x="340"" y="321" class="st3" display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 2 ? 'none' : ''}" fill="${grid_colour}">${isNaN(parseFloat(stateObj40.state)) ? 0 : parseFloat(stateObj40.state).toFixed(0)} W</text>
            <text id="noness2_value" x="320"" y="321" class="st3" display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}">${isNaN(parseFloat(stateObj40.state)) ? 0 : parseFloat(stateObj40.state).toFixed(0)} W</text>
            <text id="noness2_value" x="357" y="321" class="st3" display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}">${isNaN(parseFloat(stateObj41.state)) ? 0 : parseFloat(stateObj41.state).toFixed(0)} W</text>
            <text id="autarkye_value" x="46.5%" y="74%" display="${useautarky === false ? 'none' : ''}" class="${useautarky === 'energy' ? 'st4 st8 left-align' : 'st12'}" fill="${inverter_colour}" >${Autarky}%</text>
            <text id="ratioe_value" x="55%" y="74%" display="${useautarky === false ? 'none' : ''}" class="${useautarky === 'energy' ? 'st4 st8 left-align' : 'st12'}" fill="${inverter_colour}" >${Ratio}%</text>
            <text id="autarkyp_value" x="46.5%" y="74%" display="${useautarky === false ? 'none' : ''}" class="${useautarky === 'power' ? 'st4 st8 left-align' : 'st12'}" fill="${inverter_colour}" >${Autarkyp}%</text>
            <text id="ratiop_value" x="55%" y="74%" display="${useautarky === false ? 'none' : ''}" class="${useautarky === 'power' ? 'st4 st8 left-align' : 'st12'}" fill="${inverter_colour}" >${Ratiop}%</text>
            <text id="autarky" x="46.5%" y="77%" display="${useautarky === false ? 'none' : ''}" class="st3 left-align" fill="${inverter_colour}" >Autarky</text>
            <text id="ratio" x="55%" y="77%" display="${useautarky === false ? 'none' : ''}" class="st3 left-align" fill="${inverter_colour}" >${localize('common.ratio')}</text>
            ...
            <path id="es-load1" d="M 412 80 L 412 60" display="${show_aux === false ? '' : 'none'}" class="${additional_load === 1 ? '' : 'st12'}" fill="none" stroke="${load_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            <path id="es-load2" d="M 412 80 L 412 53" display="${show_aux === false ? '' : 'none'}" class="${additional_load === 2 ? '' : 'st12'}" fill="none" stroke="${load_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            <path id="pv1-line" d="M 86 162 L 86 56 Q 86 56 86 56 L 70 56" class="${config.show_solar === false ? 'st12' : ''}" fill="none" stroke="${solar_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            ...
            <path id="so-line" d="M 155 250 L 96 250 Q 86 250 86 240 L 86 192" class="${config.show_solar === false ? 'st12' : ''}" fill="none" stroke="${solar_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            ...
            <circle id="power-dot-charge" cx="0" cy="0" r="3" fill="${battery_power < 0 || battery_power === 0 ? 'transparent' : `${battery_colour}`}">
            ...
            <circle id="power-dot-discharge" cx="0" cy="0" r="3" fill="${battery_power > 0 || battery_power === 0 ? 'transparent' : `${battery_colour}`}">
            ...
            <path id="aux-line" d="M 307 47 L 371.5 47" fill="none" class="${show_aux === false ? 'st12' : ''}" stroke="${aux_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            <circle id="aux-dot" cx="0" cy="0" r="3" class="${show_aux === false || aux_power === 0 ? 'st12' : ''}" fill="${aux_power < 0 ? 'transparent' : `${aux_colour}`}">
            ...
            <path id="aux-line2" d="M 200 162 L 200 57 Q 200 47 210 47 L 237 47" fill="none" class="${show_aux === false ? 'st12' : ''}" stroke="${aux_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_default" x="320" y="339" width="38" height="38" viewBox="0 0 24 24" opacity="${load1_icon === 'default' && load2_icon === 'default' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="m15 13l-4 4v-3H2v-2h9V9l4 4M5 20v-4h2v2h10v-7.81l-5-4.5L7.21 10H4.22L12 3l10 9h-3v8H5Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_default_left" x="306" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load1_icon === 'default' && load2_icon != 'default' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="m15 13l-4 4v-3H2v-2h9V9l4 4M5 20v-4h2v2h10v-7.81l-5-4.5L7.21 10H4.22L12 3l10 9h-3v8H5Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_default_right" x="343" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load1_icon != 'default' && load2_icon === 'default' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="m15 13l-4 4v-3H2v-2h9V9l4 4M5 20v-4h2v2h10v-7.81l-5-4.5L7.21 10H4.22L12 3l10 9h-3v8H5Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_boiler_left" x="306" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load1_icon === 'boiler' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_boiler_right" x="343" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load2_icon === 'boiler' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_pump_left" x="306" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load1_icon === 'pump' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_pump_right" x="343" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load2_icon === 'pump' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_oven_left" x="306" y="341" width="32" height="32" viewBox="0 0 32 32" opacity="${load1_icon === 'oven' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_oven_right" x="343" y="341" width="32" height="32" viewBox="0 0 32 32" opacity="${load2_icon === 'oven' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_ac_left" x="308" y="345" width="25" height="25" viewBox="0 0 24 24" opacity="${load1_icon === 'aircon' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_ac_right" x="345" y="345" width="25" height="25" viewBox="0 0 24 24" opacity="${load2_icon === 'aircon' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" fill="${grid_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>

            <g display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 1 ? 'none' : ''}" >
            ...
            <svg xmlns="http://www.w3.org/2000/svg" id="noness1_default" x="324.5" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load1_icon === 'default' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 2 ? 'none' : ''}" fill="${grid_colour}" d="m15 13l-4 4v-3H2v-2h9V9l4 4M5 20v-4h2v2h10v-7.81l-5-4.5L7.21 10H4.22L12 3l10 9h-3v8H5Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness1_boiler" x="324.5" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load1_icon === 'boiler' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 2 ? 'none' : ''}" fill="${grid_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness1_pump" x="324.5" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load1_icon === 'pump' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 2 ? 'none' : ''}" fill="${grid_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness1_oven" x="324.5" y="341" width="32" height="32" viewBox="0 0 32 32" opacity="${load1_icon === 'oven' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 2 ? 'none' : ''}" fill="${grid_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="noness1_ac" x="326.5" y="345" width="25" height="25" viewBox="0 0 24 24" opacity="${load1_icon === 'aircon' ? '1' : '0'}"><path display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 2 ? 'none' : ''}" fill="${grid_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>

            <g display="${grid_show_noness === false || noness_dual_load === false || noness_dual_load === 2 ? 'none' : ''}">
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.aux_connected_status)}>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_default" x="371" y="5" width="83" height="83" viewBox="0 0 24 24"><path class="${aux_type === 'default' ? '' : 'st12'}" display="${show_aux === false ? 'none' : ''}" fill="${aux_status === 'on' || aux_status === '1' ? `${aux_colour}` : `${aux_off_colour}`}" d="M5 20v-8H2l10-9l10 9h-3v8H5m7-14.31l-5 4.5V18h10v-7.81l-5-4.5M11.5 18v-4H9l3.5-7v4H15l-3.5 7Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_generator" x="374" y="5" width="74" height="74" viewBox="0 0 24 24"><path class="${aux_type === 'gen' ? '' : 'st12'}" display="${show_aux === false ? 'none' : ''}" fill="${aux_status === 'on' || aux_status === '1' ? `${aux_colour}` : `${aux_off_colour}`}" d="M6 3a2 2 0 0 0-2 2v11h2v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h6v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h2V5a2 2 0 0 0-2-2H6m6 4V5h6v2h-6m0 2h6v2h-6V9M8 5v4h2l-3 6v-4H5l3-6m14 15v2H2v-2h20Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_inverter" x="388" y="8" width="44" height="69" viewBox="0 0 74 91"  preserveAspectRatio="xMidYMid meet"> <g transform="translate(0.000000,91.000000) scale(0.100000,-0.100000)" class="${aux_type === 'inverter' ? '' : 'st12'}" display="${show_aux === false ? 'none' : ''}" fill="${aux_status === 'on' || aux_status === '1' ? `${aux_colour}` : `${aux_off_colour}`}" stroke="none"> <path d="M35 887 l-27 -23 0 -404 0 -404 27 -23 c26 -23 28 -23 329 -23 284 0 305 1 327 19 l24 19 0 412 0 412 -24 19 c-22 18 -43 19 -327 19 -301 0 -303 0 -329 -23z m585 -157 l0 -80 -255 0 -255 0 0 80 0 80 255 0 255 0 0 -80z m-242 -229 c44 -34 40 -46 -14 -46 -60 0 -97 -38 -93 -94 5 -64 -23 -80 -35 -20 -9 44 24 113 63 134 35 18 34 15 21 50 -11 29 -14 30 58 -24z m110 -129 c4 -51 -19 -97 -59 -117 -27 -14 -30 -20 -23 -48 l6 -31 -51 43 c-29 24 -49 46 -46 49 3 4 23 5 44 3 58 -4 95 32 97 95 3 60 1 57 17 52 6 -3 13 -23 15 -46z"/> </g> </svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_oven" x="375" y="5" width="70" height="70" viewBox="0 0 32 32"><path class="${aux_type === 'oven' ? '' : 'st12'}" display="${show_aux === false ? 'none' : ''}" fill="${aux_status === 'on' || aux_status === '1' ? `${aux_colour}` : `${aux_off_colour}`}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_boiler" x="375" y="5" width="70" height="70" viewBox="0 0 24 24"><path class="${aux_type === 'boiler' ? '' : 'st12'}" display="${show_aux === false ? 'none' : ''}" fill="${aux_status === 'on' || aux_status === '1' ? `${aux_colour}` : `${aux_off_colour}`}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_ac" x="380" y="10" width="60" height="60" viewBox="0 0 24 24"><path class="${aux_type === 'aircon' ? '' : 'st12'}" display="${show_aux === false ? 'none' : ''}" fill="${aux_status === 'on' || aux_status === '1' ? `${aux_colour}` : `${aux_off_colour}`}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_pump" x="380" y="15" width="60" height="70" viewBox="0 0 24 24"><path class="${aux_type === 'pump' ? '' : 'st12'}" display="${show_aux === false ? 'none' : ''}" fill="${aux_status === 'on' || aux_status === '1' ? `${aux_colour}` : `${aux_off_colour}`}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>

              <g display="${show_aux === false ? 'none' : ''}" >
            ...
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_boiler_left_bottom" x="349" y="141" width="24" height="24" viewBox="0 0 24 24" opacity="${load1e_icon === 'boiler' && additional_load === 2 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_boiler_right_bottom" x="445" y="141" width="24" height="24" viewBox="0 0 24 24" opacity="${load2e_icon === 'boiler' && additional_load === 2 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_boiler_left_top" x="382" y="5" width="24" height="24" viewBox="0 0 24 24" opacity="${load1e_icon === 'boiler' && additional_load === 2 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_boiler_right_top" x="419" y="5" width="24" height="24" viewBox="0 0 24 24" opacity="${load2e_icon === 'boiler' && additional_load === 2 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_boiler_aux" x="340" y="140" width="36" height="36" viewBox="0 0 24 24" opacity="${load1e_icon === 'boiler' && additional_load === 1 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 2 ? 'none' : ''}" fill="${load_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_boiler_noaux" x="340" y="27" width="36" height="36" viewBox="0 0 24 24" opacity="${load1e_icon === 'boiler' && additional_load === 1 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 2 ? 'none' : ''}" fill="${load_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>

            <!-- Essential Aircon Icon -->
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_ac_left_bottom" x="351" y="143" width="20" height="20" viewBox="0 0 24 24" opacity="${load1e_icon === 'aircon' && additional_load === 2 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_ac_right_bottom" x="447" y="143" width="20" height="20" viewBox="0 0 24 24" opacity="${load2e_icon === 'aircon' && additional_load === 2 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_ac_left_top" x="382" y="6" width="20" height="20" viewBox="0 0 24 24" opacity="${load1e_icon === 'aircon' && additional_load === 2 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_ac_right_top" x="419" y="6" width="20" height="20" viewBox="0 0 24 24" opacity="${load2e_icon === 'aircon' && additional_load === 2 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_ac_aux" x="342" y="143" width="30" height="30" viewBox="0 0 24 24" opacity="${load1e_icon === 'aircon' && additional_load === 1 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 2 ? 'none' : ''}" fill="${load_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_ac_noaux" x="342" y="30" width="30" height="30" viewBox="0 0 24 24" opacity="${load1e_icon === 'aircon' && additional_load === 1 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 2 ? 'none' : ''}" fill="${load_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>

            <!-- Essential Pump Icon -->
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_pump_left_bottom" x="351" y="141" width="20" height="25" viewBox="0 0 24 24" opacity="${load1e_icon === 'pump' && additional_load === 2 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_pump_right_bottom" x="447" y="141" width="20" height="25" viewBox="0 0 24 24" opacity="${load2e_icon === 'pump' && additional_load === 2 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_pump_left_top" x="383" y="7" width="20" height="25" viewBox="0 0 24 24" opacity="${load1e_icon === 'pump' && additional_load === 2 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_pump_right_top" x="421" y="7" width="20" height="25" viewBox="0 0 24 24" opacity="${load2e_icon === 'pump' && additional_load === 2 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_pump_aux" x="336" y="140" width="36" height="36" viewBox="0 0 24 24" opacity="${load1e_icon === 'pump' && additional_load === 1 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 2 ? 'none' : ''}" fill="${load_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_pump_noaux" x="336" y="27" width="36" height="36" viewBox="0 0 24 24" opacity="${load1e_icon === 'pump' && additional_load === 1 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 2 ? 'none' : ''}" fill="${load_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>

            <!-- Essential Oven Icon -->
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_oven_left_bottom" x="351" y="143" width="20" height="20" viewBox="0 0 32 32" opacity="${load1e_icon === 'oven' && additional_load === 2 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_oven_right_bottom" x="447" y="143" width="20" height="20" viewBox="0 0 32 32" opacity="${load2e_icon === 'oven' && additional_load === 2 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_oven_left_top" x="382" y="5" width="24" height="24" viewBox="0 0 32 32" opacity="${load1e_icon === 'oven' && additional_load === 2 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_oven_right_top" x="419" y="5" width="24" height="24" viewBox="0 0 32 32" opacity="${load2e_icon === 'oven' && additional_load === 2 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 1 ? 'none' : ''}" fill="${load_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_oven_noaux" x="336" y="27" width="36" height="36" viewBox="0 0 32 32" opacity="${load1e_icon === 'oven' && additional_load === 1 && show_aux === false ? '1' : '0'}"><path display="${additional_load === false || additional_load === 2 ? 'none' : ''}" fill="${load_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_oven_aux" x="336" y="140" width="36" height="36" viewBox="0 0 32 32" opacity="${load1e_icon === 'oven' && additional_load === 1 ? '1' : '0'}"><path display="${show_aux === false || additional_load === false || additional_load === 2 ? 'none' : ''}" fill="${load_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>

            <g display="${additional_load === false || additional_load === 1 ? 'none' : ''}" >
              <foreignObject x="${show_aux === true ? '350' : '383'}" y="${show_aux === true ? '140' : '5'}" width="30" height="30" style="position: fixed; ">
                <body xmlns="http://www.w3.org/1999/xhtml">
                  <div style="position: fixed; ">
            ...
            <g display="${additional_load === false || additional_load === 1 ? 'none' : ''}">
              <foreignObject x="${show_aux === true ? '446' : '420'}" y="${show_aux === true ? '140' : '5'}" width="30" height="30" style="position: fixed;">
                <body xmlns="http://www.w3.org/1999/xhtml">
                  <div style="position: fixed;">
            ...
            <g display="${additional_load === false || additional_load === 2 ? 'none' : ''}">
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.use_timer_248)}>
              <svg xmlns="http://www.w3.org/2000/svg" id="timer" x="46%" y="${useautarky != false ? "60.5%" : "65%"}" width="18" height="18" viewBox="0 0 24 24"><path display="${stateObj26.state == 'on' && usetimer !== false ? '' : 'none'}" fill="${inverter_colour}" d="M15 1H9v2h6V1zm-4 13h2V8h-2v6zm8.03-6.61l1.42-1.42c-.43-.51-.9-.99-1.41-1.41l-1.42 1.42A8.962 8.962 0 0 0 12 4c-4.97 0-9 4.03-9 9s4.02 9 9 9a8.994 8.994 0 0 0 7.03-14.61zM12 20c-3.87 0-7-3.13-7-7s3.13-7 7-7s7 3.13 7 7s-3.13 7-7 7z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="timer_off" x="46%" y="${useautarky != false ? "60.5%" : "65%"}" width="18" height="18" viewBox="0 0 24 24"><path display="${stateObj26.state == 'off' && usetimer !== false ? '' : 'none'}" fill="${inverter_colour}" d="m19.95 17.15l-1.5-1.5q.275-.675.413-1.337T19 13q0-2.9-2.05-4.95T12 6q-.6 0-1.275.125t-1.4.4l-1.5-1.5q.95-.5 2.012-.763T12 4q1.5 0 2.938.5t2.712 1.45l1.4-1.4l1.4 1.4l-1.4 1.4q.95 1.275 1.45 2.713T21 13q0 1.05-.263 2.087t-.787 2.063ZM13 10.2V8h-2v.2l2 2Zm6.8 12.4l-2.4-2.4q-1.2.875-2.588 1.338T12 22q-1.85 0-3.488-.713T5.65 19.35q-1.225-1.225-1.938-2.863T3 13q0-1.5.463-2.888T4.8 7.6L1.4 4.2l1.4-1.4l18.4 18.4l-1.4 1.4ZM12 20q1.05 0 2.05-.325t1.875-.925L6.2 9.025q-.6.875-.9 1.875T5 13q0 2.9 2.05 4.95T12 20ZM9 3V1h6v2H9Zm2.075 10.875Zm2.825-2.8Z"/></svg>
              <text id="timer_text_on"x="50%" y="${useautarky != false ? "63.5%" : "68%"}" class="st3 left-align" display="${stateObj26.state == 'on' && usetimer !== false ? '' : 'none'}" fill="${inverter_colour}">Timer On</text>
              <text id="timer_text_off"x="50%" y="${useautarky != false ? "63.5%" : "68%"}" class="st3 left-align" display="${stateObj26.state == 'off' && usetimer !== false ? '' : 'none'}" fill="${inverter_colour}">Timer Off</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.solar_sell_247)}>
              <svg xmlns="http://www.w3.org/2000/svg" id="solar_sell_on" x="21%" y="51.5%" width="18" height="18" viewBox="0 0 30 30"><path display="${!config.entities.solar_sell_247 || config.entities.solar_sell_247 === 'none' || stateObj44.state === 'off' || stateObj44.state === '0' || config.show_solar === false ? 'none' : ''}" fill="${solar_colour}" d="m5.18 5.45l-1.78-.9L4.66 2h8.47l1.27 2.55l-1.78.89L11.9 4h-6l-.72 1.45M15.5 8H11l-.8-3H7.6l-.79 3H2.28L1 10.55l1.79.89L3.5 10h10.78l.72 1.45l1.79-.89L15.5 8m-.83 14H12.6l-.24-.9l-3.46-5.2l-3.47 5.2l-.23.9H3.13L6 11h2.09l-.36 1.35L8.9 14.1l1.16-1.75L9.71 11h2.07l2.89 11M8.3 15l-.9-1.35l-1.18 4.48L8.3 15m3.28 3.12l-1.18-4.48L9.5 15l2.08 3.12M23 16l-4-4v3h-4v2h4v3l4-4Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="solar_sell_off" x="21%" y="51.5%" width="18" height="18" viewBox="0 0 30 30"><path display="${!config.entities.solar_sell_247 || config.entities.solar_sell_247 === 'none' || stateObj44.state === 'on' || stateObj44.state === '1' || config.show_solar === false ? 'none' : ''}" fill="${solar_colour}" d="M 26 16 L 22 12 L 22 15 L 18 15 L 18 17 L 22 17 L 22 20 L 26 16 Z M 22.1 21.5 L 2.4 1.7 L 1.1 3 L 6.1 8 L 5.4 8 L 4.1 10.5 L 5.9 11.4 L 6.6 10 L 8.1 10 L 9.1 11 L 6.2 22 L 8.3 22 L 8.5 21.1 L 12 15.9 L 15.5 21.1 L 15.7 22 L 17.8 22 L 17 18.8 L 20.9 22.7 L 22.1 21.5 M 9.3 18.1 L 10.5 13.6 L 11.4 14.9 L 9.3 18.1 M 14.7 18.1 L 12.6 15 L 12.8 14.7 L 14.1 16 L 14.7 18.1 M 14.2 11 L 14.9 11 L 15.1 11.9 L 14.2 11 M 14.1 8 L 18.6 8 L 19.9 10.6 L 18.1 11.5 L 17.4 10 L 13.2 10 L 10.2 7 L 10.7 5 L 13.3 5 L 14.1 8 M 8.4 5.2 L 6.9 3.7 L 7.8 2 L 16.3 2 L 17.6 4.5 L 15.8 5.4 L 15 4 L 9 4 L 8.4 5.2 Z"/></svg>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.priority_load_243)}>
              <svg xmlns="http://www.w3.org/2000/svg" id="pbat" x="46%" y="${useautarky != false ? "65.5%" : "70%"}" width="18" height="18" viewBox="0 0 24 24"><path display="${stateObj25.state === 'off' && priority !== false ? '' : 'none'}" fill="${inverter_colour}" d="M15.95 21.175L13.1 18.35l1.425-1.4l1.425 1.4l3.525-3.525l1.425 1.4l-4.95 4.95ZM8 22q-.425 0-.713-.288T7 21V5q0-.425.288-.713T8 4h2V2h4v2h2q.425 0 .713.288T17 5v7q-.525 0-1.025.088T15 12.35V6H9v14h2.35q.2.575.488 1.075t.687.925H8Zm1-2h2.35H11h.35H9Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="pload" x="46%" y="${useautarky != false ? "65.5%" : "70%"}" width="18" height="18" viewBox="0 0 24 24"><path display="${stateObj25.state === 'on' && priority !== false ? '' : 'none'}" fill="${inverter_colour}" d="m15 13l-4 4v-3H2v-2h9V9l4 4M5 20v-4h2v2h10v-7.81l-5-4.5L7.21 10H4.22L12 3l10 9h-3v8H5Z"/></svg>
              <text id="priority_text_load"x="50%" y="${useautarky != false ? "68.5%" : "73%"}" class="st3 left-align" display="${stateObj25.state === 'on' && priority !== false ? '' : 'none'}" fill="${inverter_colour}">Priority Load</text>
              <text id="priority_text_batt"x="50%" y="${useautarky != false ? "68.5%" : "73%"}" class="st3 left-align" display="${stateObj25.state === 'off' && priority !== false ? '' : 'none'}" fill="${inverter_colour}">Priority Batt</text>
            </a>
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, inverter_prog.entityID)}>
              <svg xmlns="http://www.w3.org/2000/svg" id="prog_grid_on" x="58%" y="${useautarky != false ? "60.5%" : "65%"}" width="20" height="18" viewBox="0 0 24 24"><path display="${inverter_prog.show === false || config.entities.use_timer_248 === 'none' ? 'none' : ''}" class="${inverter_prog.charge === 'none' ? 'st12' : ''}" fill="${inverter_colour}" d="M11.5 19h1v-1.85l3.5-3.5V9H8v4.65l3.5 3.5V19Zm-2 2v-3L6 14.5V9q0-.825.588-1.413T8 7h1L8 8V3h2v4h4V3h2v5l-1-1h1q.825 0 1.413.588T18 9v5.5L14.5 18v3h-5Zm2.5-7Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="prog_grid_off" x="58%" y="${useautarky != false ? "60.5%" : "65%"}" width="20" height="18" viewBox="0 0 24 24"><path display="${inverter_prog.show === false || config.entities.use_timer_248 === 'none' ? 'none' : ''}" class="${inverter_prog.charge === 'none' ? '' : 'st12'}" fill="${inverter_colour}" d="M10 3H8v1.88l2 2zm6 6v3.88l1.8 1.8l.2-.2V9c0-1.1-.9-2-2-2V3h-2v4h-3.88l2 2H16zM4.12 3.84L2.71 5.25L6 8.54v5.96L9.5 18v3h5v-3l.48-.48l4.47 4.47l1.41-1.41L4.12 3.84zm8.38 13.33V19h-1v-1.83L8 13.65v-3.11l5.57 5.57l-1.07 1.06z"/></svg>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_pv_energy_108)}>
              <text id="daily_solar_value" x="43.5" y="4%" class="st10 left-align" display="${solar_showdaily === false || config.show_solar === false || remaining_solar != 'false' ? 'none' : ''}" fill="${solar_colour}" >${parseFloat(stateObj4.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_pv_energy_108)}>
              <text id="remaining_solar_value" x="43.5" y="4%" class="st10 left-align" display="${solar_showdaily === false || config.show_solar === false || remaining_solar === 'false' ? 'none' : ''}" fill="${solar_colour}" >${parseFloat(stateObj4.state).toFixed(1) || 0} kWh / ${remaining_solar} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_load_energy_84)}>
              <text id="daily_load_value_aux" x="67%" y="21%" class="st10 left-align" display="${load_showdaily === false || show_aux === false ? 'none' : ''}" fill="${load_colour}" >${parseFloat(stateObj2.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_load_energy_84)}>
              <text id="daily_load_value" x="${additional_load === false ? '83%' : '67%'}" y="${additional_load === false ? '15%' : '21%'}" class="st10 left-align" display="${load_showdaily === false || show_aux === true ? 'none' : ''}" fill="${load_colour}" >${parseFloat(stateObj2.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_battery_charge_70)}>
              <text id="daily_bat_charge_value" x="1%" y="62%" class= "st10 left-align" display="${battery_showdaily === false ? 'none' : ''}" fill="${battery_colour}">${parseFloat(stateObj1.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_battery_discharge_71)}>
              <text id="daily_bat_discharge_value" x="1%" y="70.75%" class="st10 left-align" display="${battery_showdaily === false ? 'none' : ''}" fill="${battery_colour}" >${parseFloat(stateObj.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_grid_import_76)}>
              <text id="daily_grid_buy_value" x="${grid_show_noness === false ? '68%' : '76%'}" y="${grid_show_noness === false ? '92.5%' : '62.5%'}" class="st10 left-align" display="${grid_showdailybuy === false ? 'none' : ''}" fill="${grid_colour}" >${parseFloat(stateObj3.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_grid_export_77)}>
              <text id="daily_grid_sell_value" x="${grid_show_noness === false ? '68%' : '76%'}" y="${grid_show_noness === false ? '84.25%' : '54.5%'}" class="st10 left-align" display="${grid_showdailysell === false ? 'none' : ''}" fill="${grid_colour}" >${parseFloat(stateObj33.state).toFixed(1) || 0} kWh</text>
            </a>

            ${config?.entities?.pv_total
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv_total)}>
              <text id="pvtotal_power" x="19%" y="46.5%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false ? 'none' : ''}" fill="${solar_colour}">${total_pv || 0} W</text>
            </a>`
          : svg` <text id="pvtotal_power" x="19%" y="46.5%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false ? 'none' : ''}" fill="${solar_colour}">${total_pv || 0} W</text> `}

            ${config.entities.essential_power !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.essential_power)}>
                    <text id="ess_power" x="59%" y="31%" class="${font === false ? 'st14' : 'st4'} st8" fill="${load_colour}">${essential || '0'} W</text>
                  </a>`
          : svg`<text id="ess_power" x="59%" y="31%" class="${font === false ? 'st14' : 'st4'} st8" fill="${load_colour}">${essential || 0} W</text>`}
            ${config.entities.nonessential_power !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.nonessential_power)}>
                    <text id="non_ess_power" x="74%" y="73.5%" display="${grid_show_noness === false ? 'none' : ''}" class="${font === false ? 'st14' : 'st4'} st8" fill="${grid_colour}">${nonessential || 0} W</text>
                  </a>`
          : svg`<text id="non_ess_power" x="74%" y="73.5%" display="${grid_show_noness === false ? 'none' : ''}" class="${font === false ? 'st14' : 'st4'} st8" fill="${grid_colour}">${nonessential || 0} W</text> `}
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.grid_ct_power_172)}>
              <text id="grid_ct_power_172" x="92%" y="73.5%" display="${config.entities.grid_ct_power_172 === 'none' ? 'none' : ''}" class="${font === false ? 'st14' : 'st4'} st8" fill="${grid_colour}">${grid_power || 0} W</text>
            </a>
            ${config.entities.aux_power_166 !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.aux_power_166)}>
                    <text id="aux_power_166" x="59%" y="12.5%" class="${font === false ? 'st14' : 'st4'} st8" display="${show_aux === false ? 'none' : ''}" fill="${aux_colour}">${aux_power < 0 ? aux_power * -1 : aux_power} W</text>
                  </a>`
          : svg`<text id="aux_power_166" x="59%" y="12.5%" class="${font === false ? 'st14' : 'st4'} st8" display="${show_aux === false ? 'none' : ''}" fill="${aux_colour}">${aux_power < 0 ? aux_power * -1 : aux_power} W</text> `}
            ${config.entities.pv1_power_186 !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv1_power_186)}>
                    <text id="pv1_power_186" x="8%" y="14.75%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false ? 'none' : ''}" fill="${solar_colour}" >${parseFloat(stateObj9.state).toFixed(0) || 0} W</text>
                  </a>`
          : svg`<text id="pv1_power_186" x="8%" y="14.75%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.entities.pv1_power_186 === 'none' ? 'none' : ''}" fill="${solar_colour}" >${parseFloat(stateObj9.state).toFixed(0) || 0} W</text>`}
            ${config.entities.pv2_power_187 !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv2_power_187)}>
                    <text id="pv2_power_187" x="30%" y="14.75%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj8.state).toFixed(0) || 0} W</text>
                  </a>`
          : svg`<text id="pv2_power_187" x="30%" y="14.75%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.entities.pv2_power_187 === 'none' || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj8.state).toFixed(0) || 0} W</text>`}
            ${config.entities.pv3_power_188 !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv3_power_188)}>
                    <text id="pv3_power_188" x="8%" y="30.5%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj31.state).toFixed(0) || 0} W</text>
                  </a>`
          : svg`<text id="pv3_power_188" x="8%" y="30.5%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.entities.pv3_power_188 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj31.state).toFixed(0) || 0} W</text>`}
            ${config.entities.pv4_power_189 !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv4_power_189)}>
                    <text id="pv4_power_189" x="30%" y="30.5%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj32.state).toFixed(0) || 0} W</text>
                  </a>`
          : svg`<text id="pv4_power_189" x="30%" y="30.5%" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.entities.pv4_power_189 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj32.state).toFixed(0) || 0} W</text>`}
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.inverter_voltage_154)}>
              <text id="inverter_voltage_154" x="59%" y="44.5%" display="${config.entities.inverter_voltage_154 === 'none' || !config.entities.inverter_voltage_154 ? 'none' : ''}" class="${font === false ? 'st14' : 'st4'} st8" fill="${grid_colour}" >${inverter_voltage} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.load_frequency_192)}>
              <text id="load_frequency_192" x="59%" y="49.5%" display="${config.entities.load_frequency_192 === 'none' || !config.entities.load_frequency_192 ? 'none' : ''}" class="${font === false ? 'st14' : 'st4'} st8" fill="${grid_colour}">${load_frequency} Hz</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.inverter_current_164)}>
              <text id="inverter_current_164" x="39.5%" y="52%" display="${config.entities.inverter_current_164 === 'none' || !config.entities.inverter_current_164 ? 'none' : ''}" class="${font === false ? 'st14' : 'st4'} st8" fill="${inverter_colour}">${inverter_current} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_voltage_183)}>
              <text id="battery_voltage_183" x="9%" y="82.75%" display="${config.entities.battery_voltage_183 === 'none' || !config.entities.battery_voltage_183 ? 'none' : ''}" fill=${battery_colour} class="${font === false ? 'st14' : 'st4'} st8">${battery_voltage} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_soc_184)}>
              <text id="battery_soc_184" x="29%" y="87%" display="${config.entities.battery_soc_184 === 'none' ? 'none' : ''}" fill=${battery_colour} class="st13 st8 left-align">${parseInt(stateObj12.state) || 0} %</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_soc_184)}>
              <text id="battery_soc_184" x="43%" y="87%" fill=${battery_colour} class="st13 st8 left-align" display="${inverter_prog.show === false || config.entities.battery_soc_184 === 'none' ? 'none' : ''}"> | ${inverter_prog.capacity || 0} %</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_power_190)}>
              <text id="battery_power_190" x="9%" y="93%" display="${config.entities.battery_power_190 === 'none' ? 'none' : ''}" fill=${battery_colour} class="${font === false ? 'st14' : 'st4'} st8">${battery_power < 0 ? battery_power * -1 : battery_power} W</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_current_191)}>
              <text id="battery_current_191" x="9%" y="87.75%" display="${config.entities.battery_current_191 === 'none' ? 'none' : ''}" fill=${battery_colour} class="${font === false ? 'st14' : 'st4'} st8">${stateObj35.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.inverter_power_175)}>
              <text id="inverter_power_175" x="39.5%" y="46.5%" display="${config.entities.inverter_power_175 === 'none' ? 'none' : ''}" class="${font === false ? 'st14' : 'st4'} st8" fill="${inverter_colour}">${stateObj22.state || 0} W</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.grid_power_169)}>
              <text id="grid_power_169" x="59%" y="54.5%" display="${config.entities.grid_power_169 === 'none' ? 'none' : ''}" class="${font === false ? 'st14' : 'st4'} st8" fill="${grid_colour}">${stateObj23.state || 0} W</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv1_voltage_109)}>
              <text id="pv1_voltage" x="9%" y="20.5%" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv1_voltage_109 || config.entities.pv1_voltage_109 === 'none' ? 'none' : ''}" fill="${solar_colour}" >${stateObj16.state || 0} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv1_current_110)}>
              <text id="pv1_current" x="9%" y="23.5%" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv1_current_110 || config.entities.pv1_current_110 === 'none' ? 'none' : ''}" fill="${solar_colour}" >${stateObj17.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv2_voltage_111)}>
              <text id="pv2_voltage" x="31%" y="20.5%" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv2_voltage_111 || config.entities.pv2_voltage_111 === 'none' || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}" >${stateObj18.state || 0} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv2_current_112)}>
              <text id="pv2_current" x="31%" y="23.5%" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv2_current_112 || config.entities.pv2_current_112 === 'none' || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}" >${stateObj19.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv3_voltage_113)}>
              <text id="pv3_voltage" x="9%" y="36.25%" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv3_voltage_113 || config.entities.pv3_voltage_113 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}" >${stateObj27.state || 0} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv3_current_114)}>
              <text id="pv3_current" x="9%" y="39.25%" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv3_current_114 || config.entities.pv3_current_114 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}" >${stateObj28.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv4_voltage_115)}>
              <text id="pv4_voltage" x="31%" y="36.25%" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv4_voltage_115 || config.entities.pv4_voltage_115 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}" >${stateObj29.state || 0} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv4_current_116)}>
              <text id="pv4_current" x="31%" y="39.25%" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv4_current_116 || config.entities.pv4_current_116 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}" >${stateObj30.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_temp_182)}>
              <text id="battery_temp_182" x="20.5%" y="77%" class="${config.entities.battery_temp_182 === 'none' ? 'st12' : 'st3 left-align'}" fill="${battery_colour}" display="${config?.entities?.battery_temp_182 ? '' : 'none'}" >${stateObj37.state || ''}°</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.radiator_temp_91)}>
              <text id="ac_temp" x="${config?.solar?.mppts === 4 ? "30%" : "34.5%"}"  y="${config?.solar?.mppts === 4 ? "58%" : "40%"}" class="${config.entities.radiator_temp_91 === 'none' ? 'st12' : 'st3 left-align'}" fill="${inverter_colour}" display="${config?.entities?.radiator_temp_91 ? '' : 'none'}" >AC: ${stateObj39.state || ''}°</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.dc_transformer_temp_90)}>
              <text id="dc_temp" x="24%" y="69.5%" class="${config.entities.dc_transformer_temp_90 === 'none' ? 'st12' : 'st3 left-align'}" fill="${inverter_colour}" display="${config?.entities?.dc_transformer_temp_90 ? '' : 'none'}" >DC: ${stateObj38.state || ''}°</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.energy_cost)}>
              <text id="energy_cost" x="93.5%" y="67%"  class="${config.entities.energy_cost === 'none' ? 'st12' : 'st3 left-align'}" fill="${grid_colour}" display="${config?.entities?.energy_cost ? '' : 'none'}" >${stateObj43.state || ''}</text>
            </a>
          </svg>
        </div>
        </ha-card>
    ...
    if (config.cardstyle === 'lite') {
      return html`
        ...
        <div class="container card">
          <svg viewBox="-0.5 ${config.show_solar === false ? 145.33 : -0.5} 483 ${config.show_solar === false ? 270.67 : 406}" preserveAspectRatio="xMidYMid meet" height="${panel === false ? `${config.show_solar === false ? '246px' : `${height}`}` : `${config.show_solar === false ? '75%' : '100%'}`}" width="100%"  xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <rect id="pv1 "x="154" y="54.5" width="70" height="30" rx="4.5" ry="4.5" fill="none" stroke="${solar_colour}" pointer-events="all" class="${config.show_solar === false ? 'st12' : ''}"/>
            ...
            <rect id="es-load1" x="406" y="116.5" width="70" height="30" rx="4.5" ry="4.5" fill="none" stroke="${load_colour}" pointer-events="all" display="${additional_load === 1 && config.show_solar === true ? '' : 'none'}"/>
            <rect id="es-load1" x="406" y="116.5" width="70" height="30" rx="4.5" ry="4.5" fill="none" stroke="${load_colour}" pointer-events="all" display="${additional_load === 2 && config.show_solar === true ? '' : 'none'}"/>
            <rect id="es-load2" x="406" y="290" width="70" height="30" rx="4.5" ry="4.5" fill="none" stroke="${load_colour}" pointer-events="all" display="${additional_load === 2 && config.show_solar === true ? '' : 'none'}"/>

            <text id="duration" x="290" y="377.5" class="${font === false ? 'st14' : 'st4'} left-align" fill="${config.battery.energy === 0 || float === 'True' || battery_power === 0 ? 'transparent' : `${battery_colour}`}" >${duration}</text>
            ...
            <text id="floating" x="290" y="393.7" class="st3 left-align" fill="${config.battery.energy === 0 || float === 'False' ? 'transparent' : `${battery_colour}`}" >${localize('common.battery_floating')}</text>
            <text id="daily_bat_charge" x="77.2" y="357.2" class="st3 left-align"  fill="${battery_showdaily === false ? 'transparent' : `${battery_colour}`}"  >DAILY CHARGE</text>
            <text id="daily_bat_dischcharge" x="77.2" y="393.7" class="st3 left-align"  fill="${battery_showdaily === false ? 'transparent' : `${battery_colour}`}" >DAILY DISCHARGE</text>
            <text id="daily_load" x="${additional_load === 2 ? '365' : '400.4'}" y="${additional_load === 2 ? '182.2' : '282.1'}" class="st3 left-align" fill="${load_showdaily === false ? 'transparent' : `${load_colour}`}" >${localize('common.daily_load')}</text>
            <text id="daily_grid_buy" x="5" y="282.1" class="st3 left-align" fill="${grid_showdailybuy === false ? 'transparent' : `${grid_colour}`}" >${localize('common.daily_grid_buy')}</text>
            <text id="daily_grid_sell" x="5" y="179" class="st3 left-align" fill="${grid_showdailysell === false ? 'transparent' : `${grid_colour}`}" >${localize('common.daily_grid_sell')}</text>
            <text id="daily_solar" x="200" y="40" class="st3 left-align" fill="${solar_showdaily === false || config.show_solar === false || remaining_solar != 'false' ? 'transparent' : `${solar_colour}`}">${localize('common.daily_solar')}</text>
            <text id="remaining_solar" x="200" y="40" class="st3 left-align" fill="${solar_showdaily === false || config.show_solar === false || remaining_solar === 'false' ? 'transparent' : `${solar_colour}`}" >DAILY SOLAR / LEFT TODAY</text>
            <text x="162" y="94" class="st3 st8" display="${config.show_solar === false ? 'none' : ''}" fill="${solar_colour}">${config?.solar?.pv1_name ? `${config.solar.pv1_name}` : 'PV1'}</text>
            <text x="264" y="94" class="st3 st8" display="${config.show_solar === false || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}">${config?.solar?.pv2_name ? `${config.solar.pv2_name}` : 'PV2'}</text>
            <text x="88" y="94" class="st3 st8" display="${config.show_solar === false || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}">${config?.solar?.pv3_name ? `${config.solar.pv3_name}` : 'PV3'}</text>
            <text x="340" y="94" class="st3 st8" display="${config.show_solar === false || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}">${config?.solar?.pv4_name ? `${config.solar.pv4_name}` : 'PV4'}</text>
            <text id="autarkye_value" x="130" y="260" display="${useautarky === false ? 'none' : ''}" class="${useautarky === 'energy' ? 'st4 st8 left-align' : 'st12'}" fill="${inverter_colour}" >${Autarky}%</text>
            <text id="ratioe_value" x="173" y="260" display="${useautarky === false ? 'none' : ''}" class="${useautarky === 'energy' ? 'st4 st8 left-align' : 'st12'}" fill="${inverter_colour}" >${Ratio}%</text>
            <text id="autarkyp_value" x="130" y="260" display="${useautarky === false ? 'none' : ''}" class="${useautarky === 'power' ? 'st4 st8 left-align' : 'st12'}" fill="${inverter_colour}" >${Autarkyp}%</text>
            <text id="ratiop_value" x="173" y="260" display="${useautarky === false ? 'none' : ''}" class="${useautarky === 'power' ? 'st4 st8 left-align' : 'st12'}" fill="${inverter_colour}" >${Ratiop}%</text>
            <text id="autarky" x="130" y="273" display="${useautarky === false ? 'none' : ''}" class="st3 left-align" fill="${inverter_colour}" >Autarky</text>
            <text id="ratio" x="173" y="273" display="${useautarky === false ? 'none' : ''}" class="st3 left-align" fill="${inverter_colour}" >${localize('common.ratio')}</text>
            ...
            <path id="so-line" d="M 239.23 190 L 239.22 174.02 Q 239.21 168.03 239.1 158.04 L 239 147" class="${config.show_solar === false ? 'st12' : ''}" fill="none" stroke="${solar_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            <circle id="so-dot" cx="0" cy="0" r="3" class="${config.show_solar === false ? 'st12' : ''}" fill="${totalsolar === 0 ? 'transparent' : `${solar_colour}`}">
              <animateMotion dur="${solar_animation_speed}s" repeatCount="indefinite" keyPoints="1;0" keyTimes="0;1" calcMode="linear">
                <mpath xlink:href="#so-line"/>
              </animateMotion>
            ...
            <path id="es-load1" d="M 441 180 L 441 147" class="${additional_load === 1 && config.show_solar === true ? '' : 'st12'}" fill="none" stroke="${load_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            <path id="es-load1" d="M 441 180 L 441 147" class="${additional_load === 2 && config.show_solar === true ? '' : 'st12'}" fill="none" stroke="${load_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>
            <path id="es-load2" d="M 441 290 L 441 257" class="${additional_load === 2 && config.show_solar === true ? '' : 'st12'}" fill="none" stroke="${load_colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/>

            <svg xmlns="http://www.w3.org/2000/svg" id="ess_oven_top" x="368" y="113" width="36" height="36" viewBox="0 0 32 32" opacity="${load1e_icon === 'oven' && (additional_load === 1 || additional_load === 2) ? '1' : '0'}"><path display="${additional_load === false || config.show_solar === false ? 'none' : ''}" fill="${load_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_oven_bottom" x="368" y="287" width="36" height="36" viewBox="0 0 32 32" opacity="${load2e_icon === 'oven' && additional_load === 2 ? '1' : '0'}"><path display="${additional_load === false || config.show_solar === false ? 'none' : ''}" fill="${load_colour}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_pump_top" x="368" y="113" width="36" height="36" viewBox="0 0 24 24" opacity="${load1e_icon === 'pump' && (additional_load === 1 || additional_load === 2) ? '1' : '0'}"><path display="${additional_load === false || config.show_solar === false ? 'none' : ''}" fill="${load_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_pump_bottom" x="368" y="287" width="36" height="36" viewBox="0 0 24 24" opacity="${load2e_icon === 'pump' && additional_load === 2 ? '1' : '0'}"><path display="${additional_load === false || config.show_solar === false ? 'none' : ''}" fill="${load_colour}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_ac_top" x="374" y="116" width="30" height="30" viewBox="0 0 24 24" opacity="${load1e_icon === 'aircon' && (additional_load === 1 || additional_load === 2) ? '1' : '0'}"><path display="${additional_load === false || config.show_solar === false ? 'none' : ''}" fill="${load_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_ac_bottom" x="374" y="289" width="30" height="30" viewBox="0 0 24 24" opacity="${load2e_icon === 'aircon' && additional_load === 2 ? '1' : '0'}"><path display="${additional_load === false || config.show_solar === false ? 'none' : ''}" fill="${load_colour}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_boiler_top" x="371" y="113" width="36" height="36" viewBox="0 0 24 24" opacity="${load1e_icon === 'boiler' && (additional_load === 1 || additional_load === 2) ? '1' : '0'}"><path display="${additional_load === false || config.show_solar === false ? 'none' : ''}" fill="${load_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="ess_boiler_bottom" x="371" y="287" width="36" height="36" viewBox="0 0 24 24" opacity="${load2e_icon === 'boiler' && additional_load === 2 ? '1' : '0'}"><path display="${additional_load === false || config.show_solar === false ? 'none' : ''}" fill="${load_colour}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>

            <g display="${additional_load === false || config.show_solar === false ? 'none' : ''}">
            ...
            <g display="${additional_load === false || additional_load === 1 || config.show_solar === false ? 'none' : ''}">
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.use_timer_248)}>
              ...
              <text id="timer_text_off" x="287" y="254.7" class="st3 left-align" display="${stateObj26.state == 'off' && usetimer !== false ? '' : 'none'}" fill="${inverter_colour}">Timer off</text>
              <text id="timer_text_on" x="287" y="254.7" class="st3 left-align" display="${stateObj26.state == 'on' && usetimer !== false ? '' : 'none'}" fill="${inverter_colour}">Timer on</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.priority_load_243)}>
              ...
              <text id="priority_text_batt"x="287" y="273" class="st3 left-align" display="${stateObj25.state === 'off' && priority !== false ? '' : 'none'}" fill="${inverter_colour}">Priority Batt</text>
              <text id="priority_text_load"x="287" y="273" class="st3 left-align" display="${stateObj25.state === 'on' && priority !== false ? '' : 'none'}" fill="${inverter_colour}">Priority Load</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.solar_sell_247)}>
              ...
              <svg xmlns="http://www.w3.org/2000/svg" id="solar_sell_off" x="245" y="150" width="18" height="18" viewBox="0 0 30 30"><path display="${!config.entities.solar_sell_247 || stateObj44.state === 'on' || stateObj44.state === '1' || config.show_solar === false ? 'none' : ''}" fill="${solar_colour}" d="M 26 16 L 22 12 L 22 15 L 18 15 L 18 17 L 22 17 L 22 20 L 26 16 Z M 22.1 21.5 L 2.4 1.7 L 1.1 3 L 6.1 8 L 5.4 8 L 4.1 10.5 L 5.9 11.4 L 6.6 10 L 8.1 10 L 9.1 11 L 6.2 22 L 8.3 22 L 8.5 21.1 L 12 15.9 L 15.5 21.1 L 15.7 22 L 17.8 22 L 17 18.8 L 20.9 22.7 L 22.1 21.5 M 9.3 18.1 L 10.5 13.6 L 11.4 14.9 L 9.3 18.1 M 14.7 18.1 L 12.6 15 L 12.8 14.7 L 14.1 16 L 14.7 18.1 M 14.2 11 L 14.9 11 L 15.1 11.9 L 14.2 11 M 14.1 8 L 18.6 8 L 19.9 10.6 L 18.1 11.5 L 17.4 10 L 13.2 10 L 10.2 7 L 10.7 5 L 13.3 5 L 14.1 8 M 8.4 5.2 L 6.9 3.7 L 7.8 2 L 16.3 2 L 17.6 4.5 L 15.8 5.4 L 15 4 L 9 4 L 8.4 5.2 Z"/></svg>
            </a>
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_battery_charge_70)}>
              <text id="daily_bat_charge_value" x="77.2" y="343" class= "st10 left-align" display="${battery_showdaily === false ? 'none' : ''}" fill="${battery_colour}">${parseFloat(stateObj1.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_pv_energy_108)}>
              <text id="daily_solar_value" x="200" y="26" class="st10 left-align" display="${solar_showdaily === false || config.show_solar === false || remaining_solar != 'false' ? 'none' : ''}" fill="${solar_colour}" >${parseFloat(stateObj4.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_pv_energy_108)}>
              <text id="remaining_solar_value" x="200" y="26" class="st10 left-align" display="${solar_showdaily === false || config.show_solar === false || remaining_solar === 'false' ? 'none' : ''}" fill="${solar_colour}" >${parseFloat(stateObj4.state).toFixed(1) || 0} kWh / ${remaining_solar} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_battery_discharge_71)}>
              <text id="daily_bat_discharge_value" x="77.2" y="380.1" class="st10 left-align" display="${battery_showdaily === false ? 'none' : ''}" fill="${battery_colour}" >${parseFloat(stateObj.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_load_energy_84)}>
              <text id="daily_load_value" x="${additional_load === 2 ? '365' : '400.4'}" y="${additional_load === 2 ? '168' : '267.9'}" class="st10 left-align" display="${load_showdaily === false ? 'none' : ''}" fill="${load_colour}" >${parseFloat(stateObj2.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_grid_import_76)}>
              <text id="daily_grid_buy_value" x="5" y="267.9" class="st10 left-align" display="${grid_showdailybuy === false ? 'none' : ''}" fill="${grid_colour}" >${parseFloat(stateObj3.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.day_grid_export_77)}>
              <text id="daily_grid_sell_value" x="5" y="165" class="st10 left-align" display="${grid_showdailysell === false ? 'none' : ''}" fill="${grid_colour}" >${parseFloat(stateObj33.state).toFixed(1) || 0} kWh</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.inverter_voltage_154)}>
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_voltage_183)}>
              <text id="battery_voltage_183" x="193" y="346" display="${config.entities.battery_voltage_183 === 'none' || !config.entities.battery_voltage_183 ? 'none' : ''}" fill=${battery_colour} class="${font === false ? 'st14' : 'st4'} st8">${battery_voltage} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_soc_184)}>
              <text id="battery_soc_184" x="290" y="358" display="${config.entities.battery_soc_184 === 'none' ? 'none' : ''}" fill=${battery_colour} class="st13 st8 left-align">${parseInt(stateObj12.state) || 0} %</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_soc_184)}>
              <text id="battery_soc_184" x="355" y="358" fill=${battery_colour} class="st13 st8 left-align" display="${inverter_prog.show === false || config.entities.battery_soc_184 === 'none' ? 'none' : ''}"> | ${inverter_prog.capacity || 0} %</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_power_190)}>
              <text id="battery_power_190" x="193" y="386" display="${config.entities.battery_power_190 === 'none' ? 'none' : ''}" fill=${battery_colour} class="${font === false ? 'st14' : 'st4'} st8">${battery_power < 0 ? battery_power * -1 : battery_power} W</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_current_191)}>
            <text id="battery_current_191" x="193" y="365.3" display="${config.entities.battery_current_191 === 'none' ? 'none' : ''}" fill=${battery_colour} class="${font === false ? 'st14' : 'st4'} st8">${stateObj35.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.grid_ct_power_172)}>
              <text id="grid_ct_power_172" x="135.1" y="219.2" display="${config.entities.grid_ct_power_172 === 'none' ? 'none' : ''}" class="${font === false ? 'st14' : 'st4'} st8" fill="${grid_colour}">${grid_power || 0} W</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv1_voltage_109)}>
              <text id="pv1_voltage" x="194" y="106" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv1_voltage_109 || config.entities.pv1_voltage_109 === 'none' ? 'none' : ''}" fill="${solar_colour}" >${stateObj16.state || 0} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv1_current_110)}>
              <text id="pv1_current" x="194" y="94" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv1_current_110 || config.entities.pv1_current_110 === 'none' ? 'none' : ''}" fill="${solar_colour}" >${stateObj17.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv2_voltage_111)}>
              <text id="pv2_voltage" x="296" y="106" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv2_voltage_111 || config.entities.pv2_voltage_111 === 'none' || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}" >${stateObj18.state || 0} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv2_current_112)}>
              <text id="pv2_current" x="296" y="94" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv2_current_112 || config.entities.pv2_current_112 === 'none' || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}" >${stateObj19.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv3_voltage_113)}>
              <text id="pv3_voltage" x="120" y="106" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv3_voltage_113 || config.entities.pv3_voltage_113 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}" >${stateObj27.state || 0} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv3_current_114)}>
              <text id="pv3_current" x="120" y="94" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv3_current_114 || config.entities.pv3_current_114 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}" >${stateObj28.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv4_voltage_115)}>
              <text id="pv4_voltage" x="372" y="106" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv4_voltage_115 || config.entities.pv4_voltage_115 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}" >${stateObj29.state || 0} V</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv4_current_116)}>
              <text id="pv4_current" x="372" y="94" class="st3 left-align" display="${config.show_solar === false || !config.entities.pv4_current_116 || config.entities.pv4_current_116 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}" >${stateObj30.state || 0} A</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.battery_temp_182)}>
              <text id="battery_temp_182" x="250" y="324.5" class="${config.entities.battery_temp_182 === 'none' ? 'st12' : 'st3 left-align'}" fill="${battery_colour}" display="${config?.entities?.battery_temp_182 ? '' : 'none'}" >${stateObj37.state || ''}°</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.radiator_temp_91)}>
            <text id="ac_temp" x="173" y="168.2" class="${config.entities.radiator_temp_91 === 'none' ? 'st12' : 'st3 left-align'}" fill="${inverter_colour}" display="${config?.entities?.radiator_temp_91 ? '' : 'none'}" >AC: ${stateObj39.state || ''}°</text>
            </a>
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.dc_transformer_temp_90)}>
          ...
          : svg`<text id="pvtotal_power" x="238.8" y="133.9" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false ? 'none' : ''}" fill="${solar_colour}">${total_pv || 0} W</text>`}
            ${config.entities.pv1_power_186 !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv1_power_186)}>
                    <text id="pv1_power_186" x="188.1" y="71" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj9.state).toFixed(0) || 0} W</text>
                  </a>`
          : svg`<text id="pv1_power_186" x="188.1" y="71" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.entities.pv1_power_186 === 'none' ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj9.state).toFixed(0) || 0} W</text>`}
            ${config.entities.pv2_power_187 !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv2_power_187)}>
                    <text id="pv2_power_187" x="289.5" y="71" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj8.state).toFixed(0) || 0} W</text>
                  </a>`
          : svg`<text id="pv2_power_187" x="289.5" y="71" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.entities.pv2_power_187 === 'none' || config.solar.mppts === 1 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj8.state).toFixed(0) || 0} W</text>`}
            ${config.entities.pv3_power_188 !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv3_power_188)}>
                    <text id="pv3_power_188" x="113" y="71" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj31.state).toFixed(0) || 0} W</text>
                  </a>`
          : svg`<text id="pv3_power_188" x="113" y="71" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.entities.pv3_power_188 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj31.state).toFixed(0) || 0} W</text>`}
            ${config.entities.pv4_power_189 !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.pv4_power_189)}>
                    <text id="pv4_power_189" x="366" y="71" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj32.state).toFixed(0) || 0} W</text>
                  </a>`
          : svg`<text id="pv4_power_189" x="366" y="71" class="${font === false ? 'st14' : 'st4'} st8" display="${config.show_solar === false || config.entities.pv4_power_189 === 'none' || config.solar.mppts === 1 || config.solar.mppts ===  2 || config.solar.mppts ===  3 ? 'none' : ''}" fill="${solar_colour}">${parseFloat(stateObj32.state).toFixed(0) || 0} W</text>`}
            ${config.entities.essential_power !== 'none'
          ? svg`<a href="#" @click=${(e) => this.handlePopup(e, config.entities.essential_power)}>
  ...
  setConfig(config) {
    ...
    if (!config.battery) {
      ...
      if (config.battery.show_daily === true && (!config.entities.day_battery_charge_70 || !config.entities.day_battery_discharge_71)) {
