...
export class SunsynkPowerFlowCard extends LitElement {
	...
	render() {
		...
		const stateShutdownSOCOffGrid = this.getEntity('battery.shutdown_soc_offgrid', {
		...
		const stateShutdownSOCOffGrid2 = this.getEntity('battery2.shutdown_soc_offgrid', {
		...
		const auxDynamicColour =
			this.calculateAuxLoadColour(stateAuxPower.toPower(false), Utils.toNum(config.load?.off_threshold, 0)) ||
			loadColour;
		const auxOffColour = this.colourConvert(config.load?.aux_off_colour || auxDynamicColour);
		const auxDynamicColourLoad1 =
			this.calculateAuxLoadColour(stateAuxLoad1.toPower(false), Utils.toNum(config.load?.off_threshold, 0)) ||
			loadColour;
		const auxDynamicColourLoad2 =
			this.calculateAuxLoadColour(stateAuxLoad2.toPower(false), Utils.toNum(config.load?.off_threshold, 0)) ||
			loadColour;
		...
		const dynamicColourEssentialLoad1 =
			this.calculateEssentialLoadColour(stateEssentialLoad1.toPower(false), threshold, Load1MaxThreshold) ||
			loadColour;
		const dynamicColourEssentialLoad2 =
			this.calculateEssentialLoadColour(stateEssentialLoad2.toPower(false), threshold, Load2MaxThreshold) ||
			loadColour;
		const dynamicColourEssentialLoad3 =
			this.calculateEssentialLoadColour(stateEssentialLoad3.toPower(false), threshold, Load3MaxThreshold) ||
			loadColour;
		const dynamicColourEssentialLoad4 =
			this.calculateEssentialLoadColour(stateEssentialLoad4.toPower(false), threshold, Load4MaxThreshold) ||
			loadColour;
		const dynamicColourEssentialLoad5 =
			this.calculateEssentialLoadColour(stateEssentialLoad5.toPower(false), threshold, Load5MaxThreshold) ||
			loadColour;
		const dynamicColourEssentialLoad6 =
			this.calculateEssentialLoadColour(stateEssentialLoad6.toPower(false), threshold, Load6MaxThreshold) ||
			loadColour;
		...
		const inverterVoltageL3 = config.entities?.inverter_voltage_L3
			...
			: '';
		const inverterCurrent = config.entities?.inverter_current_164 ? stateInverterCurrent.toNum(1) : 0;
		const inverterCurrentL2 = config.entities?.inverter_current_L2 ? stateInverterCurrentL2.toNum(1) : '';
		const inverterCurrentL3 = config.entities?.inverter_current_L3 ? stateInverterCurrentL3.toNum(1) : '';
		...
		const autoScaledInverterPower = config.entities?.inverter_power_175 ? stateInverterPower.toPower() : 0;
		const autoScaledGridPower = config.entities?.grid_power_169 ? stateGridPower.toPower() : 0;

		const {invert_load} = config.load;
		const loadPowerL1 = config.entities?.load_power_L1 ? stateLoadPowerL1.toPower(invert_load) : '';
		const loadPowerL2 = config.entities?.load_power_L2 ? stateLoadPowerL2.toPower(invert_load) : '';
		const loadPowerL3 = config.entities?.load_power_L3 ? stateLoadPowerL3.toPower(invert_load) : '';
		...
		switch (true) {
			case totalGridPower < -Utils.toNum(config.grid?.off_threshold, 0):
				...
				break;
			case totalGridPower >= -Utils.toNum(config.grid?.off_threshold, 0) &&
				totalGridPower <= Utils.toNum(config.grid?.off_threshold, 0):
		...
		const dynamicColourNonEssentialLoad1 =
			Math.abs(stateNonessentialLoad1.toPower(false)) > Utils.toNum(config.grid?.off_threshold, 0)
				? gridColour
				: 'grey';
		const dynamicColourNonEssentialLoad2 =
			Math.abs(stateNonessentialLoad2.toPower(false)) > Utils.toNum(config.grid?.off_threshold, 0)
				? gridColour
				: 'grey';
		const dynamicColourNonEssentialLoad3 =
			Math.abs(stateNonessentialLoad3.toPower(false)) > Utils.toNum(config.grid?.off_threshold, 0)
				? gridColour
				: 'grey';
		...
		const energy_cost_decimals =
			config.grid?.energy_cost_decimals === 0 ? 0 : config.grid?.energy_cost_decimals || 2;
		const energyCost =
			totalGridPower >= 0
		...
		const totalsolar =
			pv1PowerWatts + pv2PowerWatts + pv3PowerWatts + pv4PowerWatts + pv5PowerWatts + pv6PowerWatts;
		...
		const batteryPowerTotal = batteryCount === 2 ? batteryPower + battery2Power : batteryPower;
		...
		if (
			config.battery2.invert_flow === true ? battery2Power > 0 && !isFloating2 : battery2Power < 0 && !isFloating2
		) {
		...
		const autarkyEnergy =
			consumptionEnergy != 0
				? Math.max(Math.min(Math.round((productionEnergy * 100) / consumptionEnergy), 100), 0)
				: 0;
		const ratioEnergy =
			productionEnergy != 0
				? Math.max(Math.min(Math.round((consumptionEnergy * 100) / productionEnergy), 100), 0)
				: 0;
		...
		const autarkyPower =
			consumptionPower != 0
				? Math.max(Math.min(Math.round((productionPower * 100) / consumptionPower), 100), 0)
				: 0;
		const ratioPower =
			productionPower != 0
				? Math.max(Math.min(Math.round((consumptionPower * 100) / productionPower), 100), 0)
				: 0;
		...
		const pv1LineWidth = !config.solar.max_power
			? minLineWidth
			: this.dynamicLineWidth(pv1PowerWatts, solarMaxPower.toNum() || pv1PowerWatts, maxLineWidth, minLineWidth);
		const pv2LineWidth = !config.solar.max_power
			? minLineWidth
			: this.dynamicLineWidth(pv2PowerWatts, solarMaxPower.toNum() || pv2PowerWatts, maxLineWidth, minLineWidth);
		const pv3LineWidth = !config.solar.max_power
			? minLineWidth
			: this.dynamicLineWidth(pv3PowerWatts, solarMaxPower.toNum() || pv3PowerWatts, maxLineWidth, minLineWidth);
		const pv4LineWidth = !config.solar.max_power
			? minLineWidth
			: this.dynamicLineWidth(pv4PowerWatts, solarMaxPower.toNum() || pv4PowerWatts, maxLineWidth, minLineWidth);
		const pv5LineWidth = !config.solar.max_power
			? minLineWidth
			: this.dynamicLineWidth(pv5PowerWatts, solarMaxPower.toNum() || pv5PowerWatts, maxLineWidth, minLineWidth);
		const pv6LineWidth = !config.solar.max_power
			? minLineWidth
			: this.dynamicLineWidth(pv6PowerWatts, solarMaxPower.toNum() || pv6PowerWatts, maxLineWidth, minLineWidth);
		...
		const solarLineWidth = !config.solar.max_power
			? minLineWidth
			: this.dynamicLineWidth(totalPV, solarMaxPower.toNum() || totalPV, maxLineWidth, minLineWidth);
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) * (pv1PowerWatts / (solarMaxPower.toNum() || pv1PowerWatts));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) * (pv2PowerWatts / (solarMaxPower.toNum() || pv2PowerWatts));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) * (pv3PowerWatts / (solarMaxPower.toNum() || pv3PowerWatts));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) * (pv4PowerWatts / (solarMaxPower.toNum() || pv4PowerWatts));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) * (pv5PowerWatts / (solarMaxPower.toNum() || pv5PowerWatts));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) * (pv6PowerWatts / (solarMaxPower.toNum() || pv6PowerWatts));
		...
		if (config && config.load && config.load.animation_speed) {
			const speed =
				...
				(config.load.animation_speed - 1) *
					(Math.abs(essentialPower) / (loadMaxPower.toNum() || Math.abs(essentialPower)));
					...
					? config.battery.invert_flow === true
						? batteryPowerTotal < 0
							...
							: ((totalPV - Math.abs(batteryPowerTotal)) /
									(threePhase ? essentialPower + Math.max(auxPower, 0) : essentialPower)) *
								100
						: batteryPowerTotal > 0
							? (totalPV / (threePhase ? essentialPower + Math.max(auxPower, 0) : essentialPower)) * 100
							: ((totalPV - Math.abs(batteryPowerTotal)) /
		...
		const pvPercentageRawBat =
			totalPV === 0 || (config.battery.invert_flow === true ? batteryPowerTotal <= 0 : batteryPowerTotal >= 0)
				? 0
				: priorityLoad === 'off' || !priorityLoad
					? (totalPV / Math.abs(batteryPowerTotal)) * 100
					: ((totalPV - essentialPower) / Math.abs(batteryPowerTotal)) * 100;
		const gridPercentageRawBat =
			(config.battery.invert_flow === true ? batteryPower <= 0 : batteryPower >= 0) || totalGridPower <= 0
				? 0
				: priorityLoad === 'on'
		...
		const normalizedPvPercentage_bat =
			totalPercentageBat === 0 ? 0 : (pvPercentageRawBat / totalPercentageBat) * 100;
		const normalizedGridPercentage =
			totalPercentageBat === 0 ? 0 : (gridPercentageRawBat / totalPercentageBat) * 100;
		...
		const PV1Efficiency =
			!config.solar.pv1_max_power || config.solar.efficiency === 0
				? 100
				: Utils.toNum(Math.min((pv1PowerWatts / pv1MaxPower.toNum()) * 100, 200), 0);
		const PV2Efficiency =
			!config.solar.pv2_max_power || config.solar.efficiency === 0
				? 100
				: Utils.toNum(Math.min((pv2PowerWatts / pv2MaxPower.toNum()) * 100, 200), 0);
		const PV3Efficiency =
			!config.solar.pv3_max_power || config.solar.efficiency === 0
				? 100
				: Utils.toNum(Math.min((pv3PowerWatts / pv3MaxPower.toNum()) * 100, 200), 0);
		const PV4Efficiency =
			!config.solar.pv4_max_power || config.solar.efficiency === 0
				? 100
				: Utils.toNum(Math.min((pv4PowerWatts / pv4MaxPower.toNum()) * 100, 200), 0);
		const PV5Efficiency =
			!config.solar.pv5_max_power || config.solar.efficiency === 0
				? 100
				: Utils.toNum(Math.min((pv5PowerWatts / pv5MaxPower.toNum()) * 100, 200), 0);
		const PV6Efficiency =
			!config.solar.pv6_max_power || config.solar.efficiency === 0
				? 100
				: Utils.toNum(Math.min((pv6PowerWatts / pv6MaxPower.toNum()) * 100, 200), 0);
		...
		const areLoadsBelowThreshold = (...loads) => loads.every((load) => load.toPower(false) <= offThreshold);
