...
class SunsynkPowerFlowCard extends LitElement {
      ...
      return html`
        <div class="container card">
          <svg viewBox="-0.5 -0.5 457 383" height="${config.panel_mode === 'no' ? '396px' : '100%'}" width="100%" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <text id="daily_solar_value" x="75" y="4%" class="st10 left-align" fill="${config.solar.show_daily === 'no' || config.solar.show_solar === 'no' ? 'transparent' : `${config.solar.colour}`}" >${stateObj4.state ? stateObj4.state : '0'} kWh</text>
            ...
            <text id="daily_grid_sell_value" x="${config.grid.show_nonessential === 'no' ? '68%' : '76%'}" y="${config.grid.show_nonessential === 'no' ? '85%' : '55%'}" class="st10 left-align" fill="${config.grid.show_daily_sell === 'no' ? 'transparent' : `${config.grid.colour}`}" >${stateObj33.state ? stateObj33.state : '0'} kWh</text>
            ...
            <rect id="pv1" x="0" y="40" width="70" height="30" rx="4.5" ry="4.5" fill="none" stroke="${config.solar.colour}" pointer-events="all" class="${config.solar.show_solar === 'no' ? 'st12' : ''}"/>
            <rect id ="pv2" x="101" y="40" width="70" height="30" rx="4.5" ry="4.5" fill="none" stroke="${config.solar.colour}" pointer-events="all" class="${config.solar.show_solar === 'no' || config.solar.mppts === 'one' ? 'st12' : ''}"/>
            ...
            <path id="so-line" d="M 155 250 L 96 250 Q 86 250 86 240 L 86 192" class="${config.solar.show_solar === 'no' ? 'st12' : ''}" fill="none" stroke="${config.solar.colour}" stroke-width="1" stroke-miterlimit="10"  pointer-events="stroke"/> 
            ...
            <circle id="power-dot-charge" cx="0" cy="0" r="3" fill="${parseInt(battery_power) < 0 || parseInt(battery_power) === 0 ? 'transparent' : `${config.battery.colour}`}">
              <animateMotion dur="6s" repeatCount="indefinite" keyPoints="1;0" keyTimes="0;1" calcMode="linear">
                <mpath xlink:href="#bat-line"/>
              </animateMotion>
            </circle>
            <circle id="power-dot-discharge" cx="0" cy="0" r="3" fill="${parseInt(battery_power) > 0 || parseInt(battery_power) === 0 ? 'transparent' : `${config.battery.colour}`}">
            ...
            <circle id="grid-dot" cx="0" cy="0" r="3" fill="${parseInt(stateObj15.state) < 0 || parseInt(stateObj15.state) === 0 ? 'transparent' : `${config.grid.colour}`}">
            ...
            <path id="ne-line1" d="M 339 295 L 339 310" fill="none" stroke="${config.grid.colour}" stroke-width="1" stroke-miterlimit="10" class="${config.grid.show_nonessential === 'no' ? 'st12' : ''}" pointer-events="stroke"/>
            <circle id="ne-dot1" cx="0" cy="0" r="3" class="${config.grid.show_nonessential === 'no' ? 'st12' : ''}" fill="${nonessential <= 0 ? 'transparent' : `${config.grid.colour}`}">
              <animateMotion dur="2s" repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="linear">
                <mpath xlink:href="#ne-line1"/>
              </animateMotion>
            </circle> 
            <path id="ne-line" d="M 339 265 L 339 188" fill="none" stroke="${config.grid.colour}" stroke-width="1" stroke-miterlimit="10" class="${config.grid.show_nonessential === 'no' ? 'st12' : ''}" pointer-events="stroke"/> 
            <circle id="ne-dot" cx="0" cy="0" r="3" class="${config.grid.show_nonessential === 'no' ? 'st12' : ''}" fill="${nonessential <= 0 ? 'transparent' : `${config.grid.colour}`}">
              <animateMotion dur="4s" repeatCount="indefinite" keyPoints="1;0" keyTimes="0;1" calcMode="linear">
      ...
      return html`
        <div class="container card">
          <svg viewBox="-0.5 ${config.solar.show_solar === 'no' ? 145.33 : -0.5} 483 ${config.solar.show_solar === 'no' ? 270.67 : 406}" height="${config.panel_mode === 'no' ? `${config.solar.show_solar === 'no' ? '246px' : '396px'}` : `${config.solar.show_solar === 'no' ? '75%' : '100%'}`}" width="100%"  xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <circle id="grid-dot1" cx="0" cy="0" r="3" fill="${parseInt(stateObj15.state) < 0 || parseInt(stateObj15.state) === 0 ? 'transparent' : `${config.grid.colour}`}">
              <animateMotion dur="8s" repeatCount="indefinite" keyPoints="1;0" keyTimes="0;1" calcMode="linear">
                <mpath xlink:href="#grid-line1"/>
              </animateMotion>
            </circle>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" id="transmission_off" x="-0.5" y="187.5" width="64.5" height="64.5" viewBox="0 0 24 24"><path class="${(stateObj20.state) === 'on' || (stateObj20.state) === '1' ? 'st12' : ''}" fill="${config.grid.no_grid_colour}" d="M22.1 21.5L2.4 1.7L1.1 3l5 5h-.7l-1.3 2.5l1.8.9l.7-1.4h1.5l1 1l-2.9 11h2.1l.2-.9l3.5-5.2l3.5 5.2l.2.9h2.1l-.8-3.2l3.9 3.9l1.2-1.2M9.3 18.1l1.2-4.5l.9 1.3l-2.1 3.2m5.4 0L12.6 15l.2-.3l1.3 1.3l.6 2.1m-.5-7.1h.7l.2.9l-.9-.9m-.1-3h4.5l1.3 2.6l-1.8.9l-.7-1.5h-4.2l-3-3l.5-2h2.6l.8 3M8.4 5.2L6.9 3.7L7.8 2h8.5l1.3 2.5l-1.8.9L15 4H9l-.6 1.2Z"/></svg>
  ...
  setConfig(config) {
    ...
    if (!config.solar.mppts) {
      throw new Error('Please include the solar mppts attribute and value; one,two,three or four e.g. mppts: two');
    }
