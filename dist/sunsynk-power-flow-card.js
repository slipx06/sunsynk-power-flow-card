...
const SunsynkCardversion = '1.8.5';
console.info(
...
class SunsynkPowerFlowCard extends LitElement {
  ...
  static getStubConfig() {
    return {
      ...
      entities: {
        ...
        day_battery_charge_70: 'sensor.battery_charge_day',
        day_battery_discharge_71: 'sensor.battery_discharge_day',
  ...
  render() {
    ...
    let noness_dual_load = config?.grid?.nonessential_dual;
    if (noness_dual_load !== 'no' && noness_dual_load !== 'yes') {
      noness_dual_load = 'no';
    }
      ...
      return html`
        ...
        <div class="container card">
          ...
          <svg viewBox="-0.5 -0.5 457 383" preserveAspectRatio="xMidYMid meet" height="${panel === 'no' ? `${height}` : '100%'}" width="100%" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" id="noness_default_left" x="306" y="341" width="32" height="32" viewBox="0 0 24 24" opacity="${load1_icon === 'default' && load2_icon != 'default' ? '1' : '0'}"><path display="${grid_show_noness === 'no' || noness_dual_load === 'no'? 'none' : ''}" fill="${grid_colour}" d="m15 13l-4 4v-3H2v-2h9V9l4 4M5 20v-4h2v2h10v-7.81l-5-4.5L7.21 10H4.22L12 3l10 9h-3v8H5Z"/></svg>
            ...
            <!-- Aux Icon -->
            <svg xmlns="http://www.w3.org/2000/svg" id="aux_default" x="371" y="5" width="83" height="83" viewBox="0 0 24 24"><path class="${aux_type === 'default' ? '' : 'st12'}" display="${show_aux === 'no' ? 'none' : ''}" fill="${load_colour}" d="M5 20v-8H2l10-9l10 9h-3v8H5m7-14.31l-5 4.5V18h10v-7.81l-5-4.5M11.5 18v-4H9l3.5-7v4H15l-3.5 7Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="aux_generator" x="374" y="5" width="74" height="74" viewBox="0 0 24 24"><path class="${aux_type === 'gen' ? '' : 'st12'}" display="${show_aux === 'no' ? 'none' : ''}" fill="${load_colour}" d="M6 3a2 2 0 0 0-2 2v11h2v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h6v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h2V5a2 2 0 0 0-2-2H6m6 4V5h6v2h-6m0 2h6v2h-6V9M8 5v4h2l-3 6v-4H5l3-6m14 15v2H2v-2h20Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" id="aux_inverter" x="388" y="8" width="44" height="69" viewBox="0 0 74 91"  preserveAspectRatio="xMidYMid meet"> <g transform="translate(0.000000,91.000000) scale(0.100000,-0.100000)" class="${aux_type === 'inverter' ? '' : 'st12'}" display="${show_aux === 'no' ? 'none' : ''}" fill="${load_colour}" stroke="none"> <path d="M35 887 l-27 -23 0 -404 0 -404 27 -23 c26 -23 28 -23 329 -23 284 0 305 1 327 19 l24 19 0 412 0 412 -24 19 c-22 18 -43 19 -327 19 -301 0 -303 0 -329 -23z m585 -157 l0 -80 -255 0 -255 0 0 80 0 80 255 0 255 0 0 -80z m-242 -229 c44 -34 40 -46 -14 -46 -60 0 -97 -38 -93 -94 5 -64 -23 -80 -35 -20 -9 44 24 113 63 134 35 18 34 15 21 50 -11 29 -14 30 58 -24z m110 -129 c4 -51 -19 -97 -59 -117 -27 -14 -30 -20 -23 -48 l6 -31 -51 43 c-29 24 -49 46 -46 49 3 4 23 5 44 3 58 -4 95 32 97 95 3 60 1 57 17 52 6 -3 13 -23 15 -46z"/> </g> </svg>
      ...
      return html`
        ...
        <div class="container card">
          <svg viewBox="-0.5 ${config.show_solar === 'no' ? 145.33 : -0.5} 483 ${config.show_solar === 'no' ? 270.67 : 406}" preserveAspectRatio="xMidYMid meet" height="${panel === 'no' ? `${config.show_solar === 'no' ? '246px' : `${height}`}` : `${config.show_solar === 'no' ? '75%' : '100%'}`}" width="100%"  xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <text id="es-load1" x="445" y="157" class="st3 left-align" display="${additional_load === 'one' && config.show_solar === 'yes' ? '' : 'none'}" fill="${load_colour}" >${config?.load?.load1_name ? `${config.load.load1_name}` : 'Load 1' }</text>
            <text id="ess_load1" x="440" y="133" display="${additional_load === 'one' && config.show_solar === 'yes' ? '' : 'none'}" class="${font === 'no' ? 'st14' : 'st4'} st8" fill="${load_colour}">${parseFloat(stateObj42.state).toFixed(0) ? parseFloat(stateObj42.state).toFixed(0) : '0'} W</text>
            <text id="es-load1" x="445" y="157" class="st3 left-align" display="${additional_load === 'two' && config.show_solar === 'yes' ? '' : 'none'}" fill="${load_colour}" >${config?.load?.load1_name ? `${config.load.load1_name}` : 'Load 1' }</text>
            <text id="ess_load1" x="440" y="133" display="${additional_load === 'two' && config.show_solar === 'yes' ? '' : 'none'}" class="${font === 'no' ? 'st14' : 'st4'} st8" fill="${load_colour}">${parseFloat(stateObj42.state).toFixed(0) ? parseFloat(stateObj42.state).toFixed(0) : '0'} W</text>
            <text id="es-load2" x="445" y="330.5" class="st3 left-align" display="${additional_load === 'two' && config.show_solar === 'yes' ? '' : 'none'}" fill="${load_colour}" >${config?.load?.load2_name ? `${config.load.load2_name}` : 'Load 2' }</text>
            <text id="ess_load2" x="440" y="306.5" display="${additional_load === 'two' && config.show_solar === 'yes' ? '' : 'none'}" class="${font === 'no' ? 'st14' : 'st4'} st8" fill="${load_colour}">${parseFloat(stateObj45.state).toFixed(0) ? parseFloat(stateObj45.state).toFixed(0) : '0'} W</text>
